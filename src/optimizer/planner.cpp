...
// 智能索引前缀匹配规则：摆脱原版对 WHERE 条件顺序的强依赖，支持前缀多列匹配
bool Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds, std::vector<std::string>& index_col_names) {
    constexpr bool force_seq_scan = false;  // 彻底启用 B+ 树索引
    if (force_seq_scan) {
    ...
    int max_match_len = 0;
    std::vector<std::string> best_index_col_names;

    // 遍历表上建立的所有索引，寻找能够利用的最长前缀索引
    for (const auto& index : tab.indexes) {
        int match_len = 0;
        
        // 严格按照索引定义的列顺序，去检查 WHERE 条件中是否存在对应的等值查询
        for (const auto& index_col : index.cols) {
            bool col_found = false;
            for (const auto& cond : curr_conds) {
                // TPC-C中多列复合索引主要针对等值条件联合查询
                if (cond.is_rhs_val && cond.op == OP_EQ && cond.lhs_col.tab_name == tab_name && cond.lhs_col.col_name == index_col.name) {
                    col_found = true;
                    break;
                }
            }
        ...
        if (match_len > max_match_len) {
            ...
            best_index_col_names.clear();
            // 必须返回该索引完整的、按定义顺序排列的列名列表，供执行器获取底层的 IxIndexHandle
            for (const auto& index_col : index.cols) {
                best_index_col_names.push_back(index_col.name);
            }
        }
    }

    if (max_match_len > 0) {
        index_col_names = best_index_col_names;
        return true;
    }
    return false;
}
